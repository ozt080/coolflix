...
// Chercher trailer YouTube en français d'abord, puis anglais
$trailer = null;
foreach ($videos as $v) {
    if ($v['site'] === 'YouTube' && $v['type'] === 'Trailer' && $v['iso_639_1'] === 'fr') {
        $trailer = $v['key'];
        break;
    }
}
...
// Chercher aussi en anglais si pas de trailer FR
if (!$trailer) {
    $videosEN = tmdbGet("/$type/$id/videos?language=en-US")['results'] ?? [];
    foreach ($videosEN as $v) {
    ...
    <div class="row align-items-center">
      ...
      <div class="col-md-9">
        ...
        <div class="d-flex flex-wrap gap-2 mb-3">
          <span class="badge bg-danger px-3 py-2">★ <?php echo round($item['vote_average'] ?? 0, 1); ?>/10</span>
          <span class="badge bg-secondary px-3 py-2"><?php echo substr($type === 'movie' ? ($item['release_date'] ?? '') : ($item['first_air_date'] ?? ''), 0, 4); ?></span>
          <?php if($type === 'movie' && !empty($item['runtime'])): ?>
          <span class="badge bg-secondary px-3 py-2">⏱️ <?php echo floor($item['runtime']/60).'h'.($item['runtime']%60).'min'; ?></span>
          <?php endif; ?>
          <?php if($type === 'tv' && !empty($item['number_of_seasons'])): ?>
          <span class="badge bg-secondary px-3 py-2">📺 <?php echo $item['number_of_seasons']; ?> saison(s)</span>
          <?php endif; ?>
        </div>
        <div class="d-flex flex-wrap gap-2 mb-3">
          <?php foreach($item['genres'] ?? [] as $g): ?>
          <span class="badge" style="background:#222; border:1px solid #555; color:#fff;"><?php echo $g['name']; ?></span>
          <?php endforeach; ?>
        </div>
        ...
        <div class="d-flex gap-3 flex-wrap">
          <?php if($trailer): ?>
          <button class="btn btn-danger btn-lg px-4" onclick="lancerFilm()">
            <i class="fas fa-play me-2"></i>Regarder
          </button>
          <?php else: ?>
          <button class="btn btn-secondary btn-lg px-4" disabled>
            <i class="fas fa-play me-2"></i>Vidéo indisponible
          </button>
          <?php endif; ?>
          <a href="<?php echo $type === 'movie' ? 'films.php' : 'series.php'; ?>" class="btn btn-outline-light btn-lg px-4">
  ...
  <div class="container-fluid px-0">
    <div class="d-flex justify-content-between align-items-center px-4 py-2" style="background:#111;">
      <span class="text-white fw-bold">
        🎬 <?php echo htmlspecialchars($title); ?> — Bande-annonce
      </span>
      <button class="btn btn-sm btn-outline-danger" onclick="fermerPlayer()">
        ✕ Fermer
      </button>
    </div>
...
<main style="background:#141414; padding-bottom:60px;">
  <div class="container-fluid px-4 px-md-5 py-5">
    ...
    <div class="row g-3">
      ...
      <div class="col-6 col-sm-4 col-md-2">
        <a href="detail.php?type=<?php echo $type; ?>&id=<?php echo $s['id']; ?>" style="text-decoration:none;">
          <div class="movie-card">
            <img src="<?php echo TMDB_IMG_URL . $s['poster_path']; ?>"
                 alt="<?php echo htmlspecialchars($s['title'] ?? $s['name'] ?? ''); ?>"
                 loading="lazy">
            <div class="card-overlay">
              <p class="text-white small fw-bold mb-1"><?php echo htmlspecialchars($s['title'] ?? $s['name'] ?? ''); ?></p>
...
<script>
var trailerKey = "<?php echo $trailer; ?>";
